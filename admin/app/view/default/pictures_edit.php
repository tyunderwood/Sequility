...
<tr><td>Image :</td><td><input type="file" name="image" id="image"  size="60"  /><br />
	<?php if($fvalue['image']){ ?><div align="center"><a href="<? 
            echo HTTP_GALLERIA_LARGE."/".$fvalue['image']; ?>" 
            target="_blank">View Existing Image</a>
            <a href="javascript:void();" onclick="matchADN('<? 
                echo $fvalue['image']; ?>');" 
             >Match Image</a>
            </div><div id="ADN" ></div><?php }?></td></tr>
...
function matchADN(filename) {
      
    var url_link = '..' + '<?php echo $config_dynamic_ajax; ?>';	
    var user_id = '<?php echo $user_id; ?>';
    
    var str = 'origin=matchADN&user_id=' + user_id + '&file=' + filename;
    ...
    $.ajax( {
        ...
        error: function(json,textStatus, errorThrown) {
            alert("ajax error: "+textStatus+" error thrown: "+errorThrown);
            },        
        success: function(xml) {
    
            var result = jQuery('json', xml).text();
            ...
            for (var i=0;i<matches.length;i++) {
                adn += '<p>' + 'title: ' + json.matches[i].title + 
                       ' ;score: ' + json.matches[i].score+  
                       ' ;visualid: ' + json.matches[i].visualid +
                       ' ;user_id: ' + user_id +
                       '</p>';    
                }

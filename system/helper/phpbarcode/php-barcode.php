...
/* 
 * barcode_outimage(params, text, bars [, scale [, mode [, total_y [, space ]]]] )
 *
...
function barcode_outimage($params,$text, $bars, $scale = 1, $mode = "png", $total_y = 0, $space = ''){
...
//echo $text; exit;    
    ...
    if ($mode=='jpg' || $mode=='jpeg'){
        $filename =  dirname(__FILE__)."/tmp/code_$code.jpg";
        imagejpeg($im,$filename, 0); 
	    //header("Content-Type: image/jpeg; name=\"barcode.jpg\"");
	    //imagejpeg($im);
    } else if ($mode=='gif'){
        $filename =  dirname(__FILE__)."/tmp/code_$code.gif";
        imagegif($im,$filename, 0); 
	    //header("Content-Type: image/gif; name=\"barcode.gif\"");
	    //imagegif($im);
    } else {
        $filename =  dirname(__FILE__)."/tmp/code_$code.png";
...
function barcode_outtext($params,$code,$bars){
    $width=true;
    $xpos=$heigh2=0;
    $bar_line="";
    for ($i=0;$i<strlen($bars);$i++){
...
function barcode_outhtml($params,$code, $bars, $scale = 1, $total_y = 0, $space = ''){
    /* set defaults */
    $total_y=(int)($total_y);
    if ($scale<1) $scale=2;
    if ($total_y<1) $total_y=(int)$scale * 60;
...
/* barcode_encode_genbarcode(code, encoding, $params)
 ...
 */
function barcode_encode_genbarcode($code,$encoding,$params){
    //global $genbarcode_loc;
    $genbarcode_loc = $params['genbarcode_loc'];
...
/* barcode_encode(code, encoding, $params)
 ...
 *    array[text]     : text-positioning info
 */
function barcode_encode($code,$encoding,$params){
    ...
    $bars=barcode_encode_ean($code, $encoding,$params);
    
//print_r($bars); exit;    
    } else if (file_exists($genbarcode_loc)){
	/* use genbarcode */

	$bars=barcode_encode_genbarcode($code, $encoding, $params);
    } else {
	print "php-barcode needs an external program for encodings other then EAN/ISBN<BR>\n";
	print "<ul>\n";
...
function barcode_print($code, $encoding="ANY", $scale = 2 ,$mode = "png" ){
    ...
    $params = array('bar_color'         => array(0,0,0),
                    'bg_color'          => array(255,255,255),
                    'text_color'        => array(0,0,0),
                    'font_loc'          =>$font_loc,
                    'genbarcode_loc'    =>$genbarcode_loc,
                    'code'              =>$code);

    $bars=barcode_encode($code,$encoding,$params);
    if (!$bars) return;
    if (!$mode) $mode="png";
    if (preg_match("#^(text|txt|plain)$#i", $mode)) print barcode_outtext($params,$bars['text'],$bars['bars']);
    elseif (preg_match("#^(html|htm)$#i", $mode)) print barcode_outhtml($params,$bars['text'],$bars['bars'], $scale,0, 0);
    else barcode_outimage($params,$bars['text'],$bars['bars'],$scale, $mode);
    return $bars;
}

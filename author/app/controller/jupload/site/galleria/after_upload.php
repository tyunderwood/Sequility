...
foreach ($files as $f) {
    ...
    if ($count == 0) {
        // get uid
        $sx = explode('.',$f['fullName']);
        $user_id = end($sx);

        }
        
    file_put_contents('files/'.$user_id.'.manifest.txt',$f['fullName']."\n",FILE_APPEND);
    $count++;
...
echo("</table>\n");
file_put_contents('files/'.$user_id.'.ttlsize.txt',$ttlSize);

?>
<P><A HREF="index.php?uid=<?php echo $user_id; ?>">Upload more</A></P>

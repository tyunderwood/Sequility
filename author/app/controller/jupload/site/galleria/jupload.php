...
class JUpload {
	...
	protected function logPHPDebug($function, $msg) {
		...
		if ($this->classparams['debug_php'] === true) {
			...
			$output = "[PHP_DEBUG - $date] [$function] ".$this->arrayexpand($msg);
			file_put_contents($fname,$output,FILE_APPEND);
		}
	}
...
if ($jupart) {
	...
	if ($len > 0) {
        ...
        //$this->logPHPDebug("jupart ","created: ".$dstname);
	...
	if ($jufinal) {
		...
		if (($dlen > 0) || $this->classparams['allow_zerosized']) {
			...
			$dstfinal .= '.'.$this->user_id;
            //$this->logPHPDebug("jufinal ","created: ".$dstfinal);
    ...
    $dstfinal .= '.'.$this->user_id;
    //this->logPHPDebug("single ","created: ".$dstfinal);
...
private function page_start() {
    ...
    $this->user_id = 0;
    if ($_GET['uid']) $this->user_id = $_GET['uid'];
